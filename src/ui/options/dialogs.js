...
define((require) => {
	...
	const viewEditedBtnId = 'view-edited';
	...
	function initAddPatternDialog() {
		$('body').on('click', 'div.connector-config a', (e) => {
			e.preventDefault();

			const linkElement = e.currentTarget;
			const index = linkElement.getAttribute('data-conn');
			initPatternsList(index);
		});

		// const modalDialog = document.getElementById('conn-conf-modal');
	...
	function initViewEditedDialog() {
		const viewEditedButton = document.getElementById(viewEditedBtnId);
		viewEditedButton.addEventListener('click', async(e) => {
			e.preventDefault();
